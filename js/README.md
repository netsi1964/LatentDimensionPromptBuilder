...
This directory contains the JavaScript files for the Latent Dimension Prompt Builder, split from the original monolithic `index.html` file.
...
Some variables and functions are exposed globally on the `window` object to allow communication between components:
- `window.sample` - Current sample data
- `window.generateGridPrompt` - Grid prompt generation function
- `window.generateTextVariationsPrompt` - Text variations prompt generation function

## File Reduction

The original `index.html` file was reduced from **2,785 lines** to **491 lines** by moving:
